...
addon = xbmcaddon.Addon(id='script.module.animated_artwork')
...
def main():
    ...
    if sys.argv[1] == 'add':
        ...
        if not xbmc.getCondVisibility('Skin.String(ap_source)'):
            xbmcgui.Dialog().ok(addon_name, loc(30015))
            return
        ...
        if len(sim) == 1 and 0.9 <= sim[0].get('similarity', 0) <= 1.0:
            ...
            listitem = xbmcgui.ListItem(label=sim[0].get('file'),
                                        label2=loc(30017) + str(sim[0].get('similarity')))
            ...
            for item in similarity:
                listitem = xbmcgui.ListItem(label=item.get('file'),
                                            label2=loc(30017) + str(item.get('similarity')))
        ...
        if len(liz) == 1:
            ...
            dialog = xbmcgui.Dialog().select(loc(30010), list=liz, useDetails=True)
        ...
        if dialog > -1:
            ...
            xbmcgui.Dialog().notification(addon_name, loc(30016), xbmcgui.NOTIFICATION_WARNING)
        ...
        if jsonrpc(rpc):
            xbmcgui.Dialog().notification(addon_name, loc(30012),
                                          xbmcgui.NOTIFICATION_INFO, time=3000)
